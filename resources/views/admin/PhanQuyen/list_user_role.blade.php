                            ...
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <a href =""  type="button" data-toggle="modal" data-target="#fix{{$users->id}}"><i style="font-size:22px" class="material-icons">edit_calendar</i><a>
                                        </td>
                                        
                                    </tr>
                                    <!-- Sửa modal -->
                                    <div class="modal fade" id="fix{{$users->id}}" role="dialog">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <h4 style='color:#00b0e4' class="modal-title">Sửa Vai Trò Nhân Viên{{$users->name}}</h4>
                                            </div>
                                            <div  class="body">
                                                <form action = "{{url('nhanvien/vaitro/sua/'.$users->id)}}" id="form_validation" method="POST">
                                                @csrf
                                                    <div class="form-group form-float">
                                                        <div class="form-line">
                                                            <input  disabled type="text" class="form-control" name="name" value ="{{$users->name}}"  required>     
                                                        </div>
                                                        ...
                                                        <div class="col-sm-3">
                                                            ...
                                                            <div class="switch">
                                                                <label><input name ="role[]" value ="{{$roles->name}}" type="checkbox" @if($roles->users->contains($users->id)) checked=checked @endif><span class="lever switch-col-red"></span>{{$roles->name}}</label>
                                                            </div>
                                                        </div>
                                                        @endforeach
                                                    </div>
